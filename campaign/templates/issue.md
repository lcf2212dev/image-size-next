...
Yarn: `"resolutions": { "image-size": "npm:image-size-next@{{FORK_VERSION}}" }`. pnpm: `pnpm.overrides` with the same alias.

## Ask

Happy to open a PR for **`{{PACKAGE_NAME}}`** if useful. Thanks for maintaining open source.
